<?php
    //上传配置
    $config = array(
        "uploadPath"=>"http://acmore-file.stor.sinaapp.com/", 	  //保存路径
    ...
    if($state == "SUCCESS")
	{
		...
		$s2 = new SaeStorage();
		$result = $s2->upload('image',$tmp_file, $_FILES['picdata']['tmp_name']);//把用户传到SAE的文件转存到名为File的storage	
    ...
    //向浏览器返回数据json数据
    $file= str_replace('../','',$result);  //为方便理解，替换掉所有类似../和./等相对路径标识
